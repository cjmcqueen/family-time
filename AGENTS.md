...
### Local Development on Headless Portal TVs
If you are developing on a Portal TV without an external HDMI monitor attached (i.e. just using the device's webcam, mic, and remote), you can view the UI directly on your computer:
1. Ensure the Portal TV is powered on and connected to your local Wi-Fi.
2. Verify the ADB connection: `adb devices`
3. Launch a local screen mirroring session:
   ```bash
   scrcpy
   ```
   This opens a virtual screen on your desktop where you can interact with the Portal TV.
...
> **CRITICAL - Portal TV Banner:** If you use `LEANBACK_LAUNCHER`, you **must** also define an `android:banner` attribute on the `<application>` or `<activity>` tag (e.g., `android:banner="@mipmap/ic_launcher"`). If this is missing, the app will not appear on the Portal TV launcher at all (it won't even show a blank tile).
